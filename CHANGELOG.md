...
- `BusyOverlay` + `useDelayedBusy`: оверлей секции / fullscreen (без хедера и bottom nav).
- Новый расчёт: распознавание — оверлей на всю форму (desktop + mobile fullscreen); «Идёт распознавание» → ✓ «Готово» (500 мс).
- Новый расчёт: расчёт — оверлей на всю форму, «Считаем» → «Готово» (400 мс до показа, 500 мс перед переходом), `aria-busy`.
