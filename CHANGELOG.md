...
## 2026-06-10 (мультипозиции)

### Добавлено

- Мультипозиционный расчёт: несколько товарных строк в форме, распознавании и истории.
- `calculateMultiImportCost` — пропорциональное распределение логистики/таможни по позициям.
- `CalculationLineItemsEditor`, `CalculationMultiLineSummary`, поле `line_items` в `Calculation`.

### Изменено

- `/api/extract-file-data`: промпт возвращает `items[]` (все товарные строки).
- Карточка и «Итог» для 2+ позиций — таблица по строкам вместо одной 6-колоночной сетки.

---

## 2026-06-10 (распознавание Excel/Word)

### Добавлено

- Распознавание данных для «Новый расчёт» из **`.xlsx`** и **`.docx`**: локальное извлечение текста (`xlsx`, `mammoth`) → OpenRouter (как для OCR).
- Фикстуры: `fixtures/extract-samples/`.

### Изменено

- `/api/extract-file-data`: Excel/Word без OCR; PDF/изображения — как раньше через OCR.space.
- Промпт и fallback: первая товарная строка; qty из `1x40hc:180pcs` → 180; один контейнер при multi-FCL; CNY для China/RMB.

---

## 2026-06-10 (экспорт/импорт истории)

### Изменено

- Экспорт и импорт истории расчётов перенесены из списка в меню **⋯** в хедере страницы «История»; подписи «Экспорт истории JSON» / «Импорт истории JSON»; на десктопе (`lg+`) — модальное окно по центру (~⅓ экрана), на мобильных — снизу.

---

## 2026-06-09 (итог таблица + баннер PWA)

### Изменено

- `CalculationSummaryGrid`: одна строка 6 колонок; `clamp()` для шрифта; неразрывные пробелы в числах; горизонтальный скролл при экстремально длинных значениях.
- `InstallPrompt`: порядок кнопок **Не напоминать | Установить | Позже** (Android); iOS — две кнопки по центру.
- `next.config.ts`: `revision` для precache `/~offline` (без предупреждения Serwist).
